...
/**
 * Implements an archive based on a archive stream, but deflates the entries first into a buffer and later into a temporary
...
public abstract class AbstractStreamArchive extends AbstractArchive {
    ...
    public void open(boolean strict) throws IOException {
        ...
        try {
            ...
            if (inf.getFilter() == null) {
                log.debug("Archive stream does not contain filter definition.");
            }
            if (inf.getConfig() == null) {
                log.debug("Archive stream does not contain vault config.");
            }
            if (inf.getSettings() == null) {
                log.debug("Archive stream does not contain vault settings. using default.");
            ...
            if (inf.getProperties() == null) {
                log.debug("Archive stream does not contain properties.");
            }
            if (inf.getNodeTypes().isEmpty()) {
                log.debug("Archive stream does not contain nodetypes.");
            }
        } finally {
    ...
    /**
     * Safely returns the modification time of the archive entry or 0, if reading the time would
